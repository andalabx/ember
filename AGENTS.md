...
| `_panel(body, title, subtitle)` | Creates a styled Rich Panel with Markdown body |
...
- **Session-first CLI**: `ember` with no arguments launches the interactive REPL (`session()`). One-shot commands still work directly (`ember url ...`). The session shows a command quick-reference on startup — no need to type `help` first.
- **Persistent save config**: `ember config --save-dir <path>` writes to `~/.config/ember/config.json`. `EMBER_SAVE_DIR` env var overrides it for the current shell. All commands use `_resolve_save()` to apply the right path with explicit `-o` taking highest priority.
...
`~/.config/ember/config.json` — written by `ember config --save-dir <path>`. Currently stores one key: `save_dir`. Read at runtime by `_load_config()` in `cli.py`; never imported by library modules.
...
| `EMBER_LLM_API_KEY` | `agent.py` | API key for LLM-powered extraction. |
| `EMBER_LLM_BASE_URL` | `agent.py` | LLM API base URL (default: OpenAI). |
| `EMBER_LLM_MODEL` | `agent.py` | Model name for extraction (default: `gpt-4o-mini`). |
